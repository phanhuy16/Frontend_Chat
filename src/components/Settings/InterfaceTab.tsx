...
const WALLPAPERS = [
  ...
  { id: "gradient-blue", name: "Xanh biển", value: "linear-gradient(135deg, #1e3a8a, #1d4ed8)" },
  { id: "gradient-purple", name: "Tím mộng", value: "linear-gradient(135deg, #4c1d95, #7c3aed)" },
  { id: "pattern-dots", name: "Họa tiết", value: "radial-gradient(#ffffff22 1px, transparent 1px)", size: "20px 20px" },
];

const InterfaceTab: React.FC = () => {
  const [accentColor, setAccentColor] = useState(localStorage.getItem("theme-accent") || "#6366f1");
  const [wallpaper, setWallpaper] = useState(localStorage.getItem("chat-wallpaper") || "default");
  ...
  const updateFontSize = (size: string) => {
    ...
    // Apply logic for font size if needed globally
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 text-left">
      <div>
        <h2 className="text-xl font-black text-slate-900 dark:text-white mb-2">Giao diện & Trải nghiệm</h2>
        <p className="text-sm text-slate-500 dark:text-slate-400 mb-6 font-medium">
          Tùy chỉnh màu sắc, hình nền và hiển thị để ứng dụng trở nên cá tính hơn.
        </p>
      </div>
          ...
          <div className="flex flex-wrap gap-4">
            {ACCENT_COLORS.map((color) => (
              <button
                ...
                className={`w-12 h-12 rounded-full ${color.class} flex items-center justify-center transition-all hover:scale-110 active:scale-95 shadow-lg ${
                  accentColor === color.value ? "ring-4 ring-offset-2 ring-primary dark:ring-offset-slate-900" : ""
                }`}
                title={color.name}
              >
          ...
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
            {WALLPAPERS.map((wp) => (
              ...
              >
                ...
                />
                <span className="text-xs font-bold text-slate-600 dark:text-slate-400">{wp.name}</span>
              </button>
            ))}
          </div>
        </div>
